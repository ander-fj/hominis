  ...
  const loadMetrics = async () => {
    ...
    try {
      ...
      if (selectedPeriod !== 'all' && startDate && endDate) {
        ...
        epis = epis.filter((e: any) => e.delivery_date >= startDate && e.delivery_date <= endDate);
        exams = exams.filter((e: any) => e.exam_date >= startDate && e.exam_date <= endDate);
        incidents = incidents.filter((i: any) => i.incident_date >= startDate && i.incident_date <= endDate);
      }
      ...
      setMetrics({
        totalEmployees: employeesResCount || 0,
  ...
  const handleExportExcel = async () => {
    try {
      ...
      if (selectedPeriod !== 'all' && startDate && endDate) {
        ...
        episList = episList.filter((e: any) => e.delivery_date >= startDate && e.delivery_date <= endDate);
        examsList = examsList.filter((e: any) => e.exam_date >= startDate && e.exam_date <= endDate);
        ...
        <MRSCard title="Evolução da Taxa de Conformidade" icon={Target}>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={[
              ...
              />
              <Legend />
              <ReferenceLine y={goals.conformidade} stroke="#ef4444" strokeDasharray="3 3" label={{ value: `Meta: ${goals.conformidade}%`, position: 'right', fill: '#ef4444', fontSize: 12 }} />
              <Line
                type="monotone"
                dataKey="taxa"
                ...
                activeDot={{ r: 7 }}
              >
                <LabelList dataKey="taxa" position="top" formatter={(value: number) => `${value.toFixed(1)}%`} style={{ fill: '#6b7280', fontSize: 12 }} />
              </Line>
            </LineChart>
          </ResponsiveContainer>
        ...
        <MRSCard title="Incidentes por Mês" icon={AlertTriangle}>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={[
              ...
              />
              <Legend />
              <ReferenceLine y={goals.incidentes} stroke="#ef4444" strokeDasharray="3 3" label={{ value: `Meta Máx: ${goals.incidentes}`, position: 'right', fill: '#ef4444', fontSize: 12 }} />
              <Bar dataKey="acidentes" fill="#dc2626" name="Acidentes" radius={[8, 8, 0, 0]}>
                <LabelList dataKey="acidentes" position="top" style={{ fill: '#dc2626', fontSize: 12 }} />
              </Bar>
              <Bar dataKey="quaseAcidentes" fill="#fb923c" name="Quase-Acidentes" radius={[8, 8, 0, 0]}>
                <LabelList dataKey="quaseAcidentes" position="top" style={{ fill: '#fb923c', fontSize: 12 }} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          <div className="mt-4 flex items-center justify-between p-3 bg-gray-50 rounded-lg">
        ...
        <MRSCard title="Taxa de Conclusão de Treinamentos" icon={BookOpen}>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={[
              ...
              />
              <Legend />
              <ReferenceLine y={goals.treinamentos} stroke="#f59e0b" strokeDasharray="3 3" label={{ value: `Meta: ${goals.treinamentos}%`, position: 'right', fill: '#f59e0b', fontSize: 12 }} />
              <Line
                type="monotone"
                dataKey="taxa"
        ...
        <MRSCard title="EPIs em Boas Condições (%)" icon={HardHat}>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={[
              ...
              />
              <Legend />
              <ReferenceLine y={goals.epis} stroke="#dc2626" strokeDasharray="3 3" label={{ value: `Meta: ${goals.epis}%`, position: 'right', fill: '#dc2626', fontSize: 12 }} />
              <Line
                type="monotone"
                dataKey="taxa"
                stroke={getBarColorForValue(
