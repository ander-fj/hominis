...
const handleBarcodeScanned = async (barcode: string) => {
    ...
    console.log('[DIAGNÓSTICO] Produto não encontrado no histórico. Buscando na API Open Food Facts...');
  
    // 2. Se não achou, busca na API Open Food Facts
    try {
      // Tenta primeiro na API do Brasil
      const brApiUrl = `https://br.openfoodfacts.org/api/v2/product/${barcode}.json`;
      console.log(`[DIAGNÓSTICO] Fazendo requisição para a API do Brasil: ${brApiUrl}`);
      const brResponse = await fetch(brApiUrl, {
        headers: {
      ...
      if (brJson.status === 1 && brJson.product) {
        console.log('[DIAGNÓSTICO] Sucesso! Produto encontrado na API do Brasil.');
        const product = brJson.product;
        const productName = product.product_name_pt || product.product_name || 'Nome não encontrado';
        setNewItem({ name: productName, quantity: '1', unit_price: '', barcode: barcode });
        Alert.alert('Produto Encontrado!', `Produto: ${productName}\n\nAgora preencha o preço e a quantidade.`);
      } else {
        // Se não encontrou no Brasil, tenta na API Mundial (fallback)
        console.log('[DIAGNÓSTICO] Produto não encontrado no Brasil. Buscando na API Mundial...');
        const worldApiUrl = `https://world.openfoodfacts.org/api/v2/product/${barcode}.json`;
        console.log(`[DIAGNÓSTICO] Fazendo requisição para a API Mundial: ${worldApiUrl}`);
        const worldResponse = await fetch(worldApiUrl, {
          headers: {
            'User-Agent': 'ShoppingListApp/1.0 (seu-email@exemplo.com)',
          },
        });
        ...
        if (worldJson.status === 1 && worldJson.product) {
            console.log('[DIAGNÓSTICO] Sucesso! Produto encontrado na API Mundial.');
            const product = worldJson.product;
            const productName = product.product_name_pt || product.product_name || 'Nome não encontrado';
            setNewItem({ name: productName, quantity: '1', unit_price: '', barcode: barcode });
            Alert.alert('Produto Encontrado!', `Produto: ${productName}\n\nAgora preencha o preço e a quantidade.`);
        } else {
            console.log('[DIAGNÓSTICO] Produto também não foi encontrado na API Mundial.');
      ...
      console.error('[ERRO GRAVE] Falha ao buscar no Open Food Facts:', error);
      setNewItem({ name: '', quantity: '1', unit_price: '', barcode: barcode });
