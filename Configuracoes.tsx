...
import { Save, Plus, Trash2, GripVertical, RefreshCw, FileSpreadsheet, Download, Upload, AlertTriangle, User } from 'lucide-react';
...
import { recalculateAllRankingsEngine } from '@/lib/rankingEngine';
import * as XLSX from 'xlsx';
import { collection, getDocs, doc, addDoc, deleteDoc, writeBatch, getDoc, query, where, updateDoc } from 'firebase/firestore';
...
export default function Configuracoes() {
  ...
  const loadCriteria = async () => {
    ...
    try {
      ...
      const q = query(criteriaCol, where('companyId', '==', companyId));
      const querySnapshot = await getDocs(q);
      const data = querySnapshot.docs
        .map(doc => ({ id: doc.id, ...doc.data() } as Criterion))
        .sort((a, b) => a.display_order - b.display_order);
      if (data) setCriteria(data);
    } catch (error) {
      console.error("Erro ao carregar critérios:", error);
      // Não alertamos aqui para não bloquear a UI, mas o erro fica no console
    }
  };

  const loadSheetConfig = async () => {
    // This table seems specific to a Supabase feature (e.g., via Edge Functions).
    // In Firebase, this would likely be managed in a specific document or as part of a Cloud Function's configuration.
    // For now, we'll read from a specific document, e.g., in a 'config' collection.
    if (!companyId) return;
    const docRef = doc(db, 'companies', companyId, 'config', 'sheets_sync');
  ...
  const loadSyncPages = async () => {
    if (!companyId) return;
    try {
      const syncPagesCol = collection(db, 'sheets_sync_pages');
      const q = query(syncPagesCol, where('companyId', '==', companyId));
      const querySnapshot = await getDocs(q);
      const data = querySnapshot.docs
        .map(doc => ({ id: doc.id, ...doc.data() } as SyncPage))
        .sort((a, b) => a.page_name.localeCompare(b.page_name));
      setSyncPages(data);
    } catch (error) {
      console.error("Erro ao carregar páginas de sincronização:", error);
    }
  };

  const togglePageEnabled = async (pageId: string, currentValue: boolean) => {
    const docRef = doc(db, 'sheets_sync_pages', pageId);
    await updateDoc(docRef, { is_enabled: !currentValue });
    await loadSyncPages();
  };

  const toggleAccumulateData = async (pageId: string, currentValue: boolean) => {
    const docRef = doc(db, 'sheets_sync_pages', pageId);
    await updateDoc(docRef, { accumulate_data: !currentValue });
    await loadSyncPages();
  };
  ...
  const handleSyncFromSheets = async () => {
    ...
    try {
      ...
      // The original logic was likely a Supabase Edge Function call.
      // This would need to be replaced with a call to a Firebase Cloud Function.
      for (const page of enabledPages) {
        await updateDoc(doc(db, 'sheets_sync_pages', page.id), {
  ...
  const handleAdd = async () => {
    if (!companyId) return;
    const newCriterion: Omit<Criterion, 'id'> = {
      name: 'Novo Critério',
      description: '',
      data_type: 'numeric',
      weight: 0,
    ...
    if (docRef.id) {
      setCriteria([...criteria, { ...newCriterion, id: docRef.id }]);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      for (const criterion of criteria) {
        ...
        await updateDoc(doc(db, 'evaluation_criteria', id), {
  ...
  const handleRecalculate = async () => {
    ...
    try {
      ...
      console.error('Erro detalhado ao recalcular:', error);
      alert(`Erro ao recalcular rankings: ${error.message || 'Verifique o console para detalhes'}`);
    } finally {
  ...
  const handleDeleteAllData = async () => {
    ...
    setDeletingAll(true);
    if (!companyId) {
      alert("ERRO CRÍTICO: Não foi possível identificar a empresa. A operação foi cancelada para sua segurança.");
      setDeletingAll(false);
      return;
    }

    try {
      // This replaces the Supabase RPC call 'truncate_all_data'.
      // WARNING: This is a client-side operation and can be slow and fail on large datasets
      // due to security rules or timeouts. A Firebase Cloud Function is the recommended approach for production.
      const collectionsToDelete = [
        'employees', 'employee_scores', 'sst_trainings', 'sst_ppe',
        'sst_medical_exams', 'sst_incidents', 'vacation_records', 'employee_rankings',
        'evaluation_criteria', 'sheets_sync_pages', 'sst_goals'
      ];
...
│ ABA 7: "Ferias"                                                         │
├─────────────────────────────────────────────────────────────────────────┤
│ Sincroniza: Períodos de Férias dos Colaboradores                        │
│ Modo recomendado: ACUMULAR (registro contínuo)                         │
│                                                                         │
│ Colunas obrigatórias:                                                  │
│   • employee_id                                                        │
...
│   • status (texto: "Aprovado", "Pendente", "Rejeitado")                │
│                                                                         │
│ Exemplo de dados:                                                      │
│ ┌────────────┬─────────────┬─────────────┬───────────┐│
│ │ employee_id│ data_inicio │ data_fim    │ status    ││
│ ├────────────┼─────────────┼─────────────┼───────────┤│
│ │ ana.oliveira@empresa.com │ 20/12/2025  │ 10/01/2026  │ Aprovado  ││
│ │ carlos.santos@empresa.com│ 15/11/2025  │ 30/11/2025  │ Aprovado  ││
│ └────────────┴─────────────┴─────────────┴───────────┘│
└─────────────────────────────────────────────────────────────────────────┘
...
CENÁRIO 4: Final do Mês
  ✓ Marcar "Avaliacoes", "Treinamentos", "EPIs", "Ferias"
  ✓ Modo: Acumular
...
═══════════════════════════════════════════════════════════════════════════
  ...
  const handleUploadExcel = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (!companyId) { alert("Não foi possível identificar a empresa. Faça login novamente."); return; }
    ...
    try {
      ...
      if (workbook.SheetNames.includes('Colaboradores')) {
        ...
        if (rows.length > 0) {
          ...
          // Firestore doesn't have a bulk upsert. We do it one by one.
          for (const employee of employees) {
            const q = query(collection(db, 'employees'), where('email', '==', employee.email));
            const existing = await getDocs(q);
            if (existing.empty) {
              await addDoc(collection(db, 'employees'), employee);
            } else {
              await updateDoc(doc(db, 'employees', existing.docs[0].id), employee);
            }
          }
      ...
      if (workbook.SheetNames.includes('Avaliacoes')) {
        ...
        if (rows.length > 0) {
          for (const row of rows) {
            const email = row.employee_id ? String(row.employee_id).trim() : '';
            ...
            if (employee) {
              ...
              // Tenta mapear colunas do Excel para critérios existentes
              criteria.forEach(criterion => {
                const normalize = (str: string) => str.toLowerCase().replace(/[^a-z0-9]/g, '');
                const criterionName = normalize(criterion.name);
                const matchingKey = Object.keys(row).find(key => normalize(key) === criterionName);
                
                if (matchingKey && row[matchingKey] !== undefined) {
                  scores[criterion.id] = Number(row[matchingKey]);
                }
              });

              if (Object.keys(scores).length > 0) {
                await addDoc(collection(db, 'employee_scores'), {
                ...
                console.log(`   ✅ Avaliação inserida para ${row.employee_id}`);
                successCount++;
              }
            } else {
      ...
      if (workbook.SheetNames.includes('Treinamentos')) {
        ...
        if (rows.length > 0) {
          for (const row of rows) {
            const q = query(collection(db, 'employees'), where('email', '==', row.employee_id));
            const snapshot = await getDocs(q);
            const employee = snapshot.empty ? null : { id: snapshot.docs[0].id, ...snapshot.docs[0].data() };
            ...
            if (employee) {
              ...
              try {
                await addDoc(collection(db, 'sst_trainings'), {
                    ...
                    expiry_date: completionDate, // Assuming same as completion for simplicity
            ...
            if (employee) {
              ...
              try {
                await addDoc(collection(db, 'sst_ppe'), {
                  ...
                  expiry_date: deliveryDate, // Assuming same as delivery for simplicity
                  status: 'delivered',
      ...
      // Processar aba Ferias
      if (workbook.SheetNames.includes('Ferias')) {
        console.log('📋 Processando aba Ferias...');
        const sheet = workbook.Sheets['Ferias'];
        const rows = XLSX.utils.sheet_to_json(sheet) as any[];
        console.log(`   Encontradas ${rows.length} linhas`);
        ...
        if (rows.length > 0) {
          for (const row of rows) {
            ...
            if (employee) {
              const startDate = parseExcelDate(row.data_inicio);
              const endDate = parseExcelDate(row.data_fim);
              try {
                await addDoc(collection(db, 'vacation_records'), {
                    employee_id: employee.id,
                    companyId: companyId,
                ...
                console.log(`   ✅ Férias inseridas para ${row.employee_id}`);
                successCount++;
              } catch (error) {
                console.error(`   ❌ Erro ao inserir férias:`, error);
                errors.push(`Férias: ${error.message}`);
                errorCount++;
              }
            } else {
              console.warn(`   ⚠️ Funcionário não encontrado: ${row.employee_id}`);
              errors.push(`Férias: Funcionário ${row.employee_id} não encontrado`);
              errorCount++;
            }
          }
        }
      } else {
        console.log('⚠️ Aba "Ferias" não encontrada');
      }
  ...
  const handleDownloadSampleData = () => {
    ...
    const feriasData = [
      ['employee_id', 'data_inicio', 'data_fim', 'status'],
      ['ana.oliveira@empresa.com', '2025-12-20', '2026-01-10', 'Aprovado'],
      ['carlos.santos@empresa.com', '2025-11-15', '2025-11-30', 'Aprovado'],
    ];
    ...
    const wsFerias = XLSX.utils.aoa_to_sheet(feriasData);
  ...
  return (
    <div className="space-y-6">
      ...
      <MRSCard title="Importação de Dados em Massa" subtitle="Importe colaboradores, avaliações e outros dados via Excel">
        <div className="space-y-4">
          <p className="text-sm text-gray-700">
            Use nosso template Excel para importar múltiplos registros de uma só vez. Siga as instruções para garantir que os dados sejam importados corretamente.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <button
              onClick={handleDownloadTemplate}
              className="w-full px-4 py-3 bg-gray-100 text-gray-800 rounded-lg hover:bg-gray-200 transition-all flex items-center justify-center gap-2 font-semibold"
            >
            ...
            <button
              ...
              disabled={uploading}
              className="w-full px-4 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:shadow-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2 font-semibold"
            >
